...
class MyoClient:
    ...
    async def on_emg_data(self, emg: EMGData, timestamp):  # data: list of 8 8-bit unsigned short
        raise NotImplementedError()
    ...
    async def notify_callback(self, sender: BleakGATTCharacteristic, data: bytearray):
        ...
        #print(f"notify_callback ({handle}): {data}")
        if handle == Handle.CLASSIFIER_EVENT:
            ...
            if self.aggregate_all:
                ...
                await self.on_imu_data(IMUData(data), receive_time)
            ...
            if self.aggregate_emg:
                ...
                await self.on_emg_data(emg, receive_time)
